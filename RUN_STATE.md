...
**PROGRESS THIS SESSION (the wall is broken):**
- ✅ **The perturbation OPERATOR is CRACKED + verified to machine precision** (`hka_pert_hka99.py`, gauge-mode gate `|res|/|Ψ|`≈**1e-10** for all κ̄). Root cause found by fetching the authoritative long paper (**gr-qc/9607010 §V**, `HKA99_src/rflanl.tex`): the transcribed coefficients (5.5–5.10) were *correct all along*; the bug was the **ASSEMBLY** — the paper writes `M_x·Ψ' = (Gmat − κ M_s)Ψ` with **M_x ≠ I** (fluid block `[[Ax,Bx],[Cx,Dx]]`), so `L = M_x⁻¹(Gmat − κ M_s)`. This is the exact wall that blocked the overnight run + this session's first pass. (D-029 to be written.)
- ⏳ **Eigenvalue extraction (κ₀≈2.81 → β): machinery built, blocked at the sonic-point numerics.** The identity (eq:alg-PP) is not preserved by the 4-row ODE → use the identity-reduced 3-D system (built, `hka_beta_match.py`); but the sonic point (regular singular) has a violent +7 eigenvalue and the Frobenius series diverges for |t|≳0.3 → direct shoot/collocation recover only the gauge mode (κ≈1 in this bg's gauge), the physical κ=2.81 is masked. **β NOT measured, none faked (D-016/D-021).** Precise handoff in `RESULTS_hka_beta.md` (2026-07-13 update).
- **NEXT (well-scoped):** reformulate in **double-null / CSS-similarity coordinates** (tournament F1) where the self-similar solution is smooth-on-grid and the sonic instability is tamed → the eigenvalue drops out (discard the gauge root at κ≈1). Then C++ port + β golden + `fluidcss_nexus` v1.0.0. Then scalar-DSS **γ=0.374** crown = new `similarity_nexus` contract.
