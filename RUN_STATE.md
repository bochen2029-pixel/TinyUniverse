...
- 🔴 **KEY FINDING: κ=2.81 is ABSENT from the operator (6 methods agree).** Implemented HKA's *own* method verbatim (`rflanl.tex` §V 5.216–5.463; `nr_shoot.py`): unique analytic-at-sonic solution (ker R + identity + gauge N̄_p(0)=0) integrated sonic→center, kill the `e^{−2x}` expanding mode. On the exact Frobenius + **verified** identity (`idc·Ψ_g≈1e-16`), the ONLY eigenvalue in **κ∈[−1.5,12] real + complex nbhd** is the **gauge mode κ=1** (positive control: reader resolves it to κ=0.99999924). No physical κ=2.81 anywhere. Structural checks all PASS (gauge gate 1e-10, center indicial {−1,0,0,−2}=eq:PPasmp1, sonic indicial 1−2κ) → **the gauge gate is necessary but NOT sufficient; D-029's "verified" is AMENDED (D-030).**
- **NEXT (redirected — the wall is the OPERATOR, not the numerics):** verify `hka_pert_hka99` *beyond the gauge mode* — a 2nd independent operator test, or re-derive the gauge-under-constrained ∂_s coefficients (`As,Bs,Cs,Ds`, `E3,F3`) from the primary KHA95 EOM keeping ∂_s. Then re-run `nr_shoot` (the validated reader) — κ=2.81 falls out with no further numerics once the operator is right. Then C++ port + β golden + `fluidcss_nexus` v1.0.0. **β NOT measured, none faked (D-016/D-021).**
