...
## CURRENT DIRECTIVE (2026-07-12, this session)

**Axis C — the CSS×spectral crown** (operator picked the γ-tournament's actual winner over the stale "AMR contract" label; reconciled in **D-028**: AMR-for-N0 was graveyarded on budget → reinstates at N4-GPU; the γ route is the **eigenvalue γ=1/Re λ₀** in similarity coordinates, and D-021's uniform-grid wall is amended to a *gauge×discretization* artifact, not a grid law). **Warm-up first** (the ruling): land the known radiation-fluid **β=0.35580192** by closing `fluidcss_nexus` **Stage-B** — the live wall is the perturbation operator's **κ-coupling K** (L=J3+κK fails the gauge-mode exactness gate; the gauge constraints determine 6 of K's 9 entries, 3 dof = the genuine ∂ₛ content, from the primary source / a re-derivation — `RESULTS_hka_beta.md`). Then the scalar-DSS **γ=0.374** crown gets a new `similarity_nexus` contract. **No new contract needed for Stage-B** (already scoped in `fluidcss_nexus.contract.md` v0.9.1).
