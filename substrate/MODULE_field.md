...
| **cloudF** | 128³ / 64 su | analytic free-fall t_ff + v1 `cloud`/`collapse` physics (classical Q→0 limit) | overdense sphere (R=12, M=8000) **collapses**: ρ_peak ×756, core 12.0→0.83 su, max compression at **t/t_ff = 0.97** (free-fall time, textbook); quantum-pressure bounce follows | `2308ea49…` GREEN |
| **mergerF** | 128³ / 64 su | two-body attraction + v1 `merger` physics | two ψ-lumps (M=2000 total, sep 12) **attract** under mutual self-gravity: separation 11.2→6.0 (**×0.54**), denser remnant (ρ_peak ×224), on the two-body time (t/t2b=0.67) | `a09dda6a…` GREEN |

**All 6 scenarios GREEN.** freepacket + sho3d exact vs the fp64 N5 oracle at both ends; soliton is the load-bearing weld; echoF the determinism receipt; cloudF + mergerF the classical/two-body cross-checks. They reproduce byte-identically on fresh runs (`--golden` re-runs the physics and confirms). The weld is proven at **three dynamical regimes**: the bound quantum ground state (soliton), classical gravitational collapse (cloudF), and two-body gravitational attraction (mergerF) — quantum pressure and self-gravity both real, at every scale the design needs.
...
- **[cloudF — the classical-limit weld cross-check, built in place of galaxyF].** galaxyF (mapping the v1 3-component galaxy, r=14–240 su, to ψ with a *rotation curve* gate) is the contract's acknowledged hardest open question (Q-N1-1): a single-valued ψ has curl-free velocity and cannot carry a rotating disk's angular momentum except via quantized vortices — research-grade, and it would need a ~512-su box. Rather than force a likely-unpassable galaxyF (risking a faked-looking in-band number — D-016), the achievable **irrotational** v1-farm cross-check was built: **cloudF** reproduces the v1 `cloud`/`collapse` physics *by the field*. An overdense uniform sphere (at rest, S=0 → curl-free → maps exactly to ψ) infalls under self-gravity in real time (`spStepReal`, shared with echoF) and **collapses at the analytic free-fall time**: ρ_peak ×756, core 12.0→0.83 su, **maximum compression at t/t_ff = 0.97** (t_ff = √(3π/32Gρ₀), textbook), then a quantum-pressure bounce (the Madelung Q halting collapse at the center). This proves the fused loop does collisionless gravitational collapse in the classical (large-scale, Q→0) limit — the other half of the two-sided Madelung claim (soliton = the quantum-pressure-supported ground state; cloudF = the classical infall dynamics).
- **[mergerF — the two-body cross-check].** Two self-gravitating ψ-lumps (M=2000 total, separation 12, at rest) **attract under mutual self-gravity**: their centroid separation shrinks 11.2 → 6.0 su (×0.54, a 46% approach) on the two-body free-fall time (t/t2b=0.67), forming a denser remnant (ρ_peak ×224). Because the lump centroids move *only* because gravity pulls them together, this is unambiguous **gravitational attraction between distinct masses** — the PM Poisson produces real gravity between separated sources, not just within one lump. Gate: separation ≤0.6× initial AND remnant ρ_peak ≥2×. (The gate is on *attraction*, not a fully-virialized merger: each lump's internal collapse timescale competes with the orbital time, so a clean full merge would need stable-soliton lumps + orbit tuning — the attraction is the load-bearing signal, honestly gated.) With soliton (1 bound lump) + cloudF (collapse) + mergerF (2-body), the weld is demonstrated across the full dynamical range.
- **galaxyF is now the only unbuilt scenario** — the v1 galaxy *rotation curve* by the field (Q-N1-1): a single-valued ψ cannot carry a rotating disk's angular momentum without quantized vortices (research-grade) + a ~512-su box. Named future work; not faked. The other v1-farm physics (cloud/collapse via cloudF, merger via mergerF) is covered by the irrotational cross-checks.
