...
| **cloudF** | 128³ PERIODIC, box 64 | **[AS BUILT — the achievable irrotational v1 cross-check]** overdense uniform sphere (R=12, M=8000, S=0 → curl-free → maps exactly to ψ), real-time self-gravity | 3000 | **collapse at the free-fall time**: ρ_peak enhancement ≥3× AND collapse time ∈ [0.4, 3]·t_ff where t_ff=√(3π/32Gρ₀). **[MEASURED]** ρ_peak ×756, core 12.0→0.83 su, max compression at t/t_ff = **0.97**; mass conserved | **analytic free-fall + v1 `cloud`/`collapse`** — cross-check · GREEN `2308ea49` |
| **galaxyF** | 256³ PERIODIC | v1 `galaxy` morphology mapped to ρ+phase (rotational support via |ψ|² structure + phase winding), small ħ/m | 10⁴ (v1 parity) | rotation curve reproduced: `v_circ(r)` from the field within **8%** of the v1 `galaxy` golden band · mass conserved (`\|M(t)−M(0)\|/M < 1e-3`, periodic → norm exactly conserved) | **v1 galaxy golden** (`2642d510`, D-019) — cross-check. **[DEFERRED — Q-N1-1: a single-valued ψ has curl-free velocity; a rotating disk needs quantized vortices (research-grade) + a ~512-su box. cloudF is the built irrotational classical cross-check instead; galaxyF is named future work rather than a faked in-band number (D-016).]** |
| **mergerF** | 128³ PERIODIC, box 64 | **[AS BUILT]** two ψ-lumps (M=2000 total, sep 12, at rest → curl-free) under mutual self-gravity | 4000 | **gravitational attraction**: centroid separation shrinks to ≤0.6× initial AND remnant ρ_peak ≥2× (the honest robust signal — the lumps move *only* because gravity pulls them). **[MEASURED]** sep 11.2→6.0 (×0.54), ρ_peak ×224, t/t2b=0.67. Gate is on *attraction*, not full virialization (internal-collapse vs orbital timescale — full merge is future work) | **v1 merger + two-body free-fall** — cross-check · GREEN `a09dda6a` |
...
- v1.0.2 (2026-07-12) — **cloudF + mergerF added → 6/6 scenarios GREEN (galaxyF is the one deferred, Q-N1-1).** The weld is now demonstrated across THREE dynamical regimes: bound quantum ground state (soliton), classical gravitational collapse at the free-fall time (cloudF, `2308ea49`), and two-body gravitational attraction (mergerF, `a09dda6a` — two lumps fall together, sep ×0.54). galaxyF (rotation curve) remains named future work (ψ can't carry disk rotation without vortices + a 512-su box; not faked).
