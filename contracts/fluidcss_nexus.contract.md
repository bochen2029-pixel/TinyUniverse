# CONTRACT — fluidcss_nexus (radiation-fluid CSS critical exponent, eigenvalue route) · v1.0.0 · status: FROZEN 2026-07-19 (**COMPLETE — β MEASURED**: Stage-A = the TRUE Evans–Coleman background `27af7920` + Stage-B β golden `9f8587fd`; κ₀=2.810577 → β=0.3557988 vs lit 0.35580192 — G-ANCHOR/G-CONVERGE/G-UNIQUE all fire; D-032. The v0.9.x Stage-A golden `b4f4e463` banked the FRIEDMANN solution mislabeled as EC — superseded, `goldens/fluidcss_stageA/NOTE.md`)
...
- v1.0.0 (2026-07-19) — **COMPLETE: β MEASURED (D-032).** The v0.9.x "Evans–Coleman background" was the collapsing flat **FRIEDMANN** solution (its sonic criterion V=−c_s is precisely the Friedmann point of the rflanl 4.7–4.9 V₀-parametrized sonic line; measured V=−√(1−1/A) to 1.9e-10 — the FRW free-fall identity). Stage-A REBUILT as the paper's own construction: sonic values (4.7–4.9) at free V₀, order-1 quadratic branch, sonic→center shoot, bisection on the case1(A<1)/case2(second-sonic) transition → **V₀\*=0.1124394014**, N̄'(sonic)=−0.355699 (the paper's sonic-gauge fingerprint; its printed "0.35699" is a dropped-digit typo), exactly one V-zero (Bogoyavlensky index 1 = EC), center-relaunch closure 4.2e-6 → golden `27af7920` (supersedes `b4f4e463`, NOTE.md). Stage-B BUILT: sonic Frobenius (a₀∈ker R + identity eq:alg-PP + gauge N̄_p(0)=0, order-18 recursion) → sonic→center shoot on the stored background → deterministic κ-bisections: **κ₀=2.810577211 → β=1/κ₀=0.355798800** (lit 2.8105525488/0.35580192; |Δβ|=3.1e-6 ≪ the 4e-3 G-ANCHOR). CONTROLS: sonic-gauge mode 0.355740 (analytic −N̄'_ss=0.355699 ✓) + origin-gauge 0.999954 (exact 1 ✓). G-CONVERGE spread 1.4e-8 (dx/2 + t₀ variation); G-UNIQUE exactly 1 root in κ∈(1.1,3.6). Golden `fluidcss_stageB` `9f8587fd` frozen, two-passed. Python cross-validation at higher precision: Lyapunov one-step-map spectrum κ=2.8105526 + shoot κ=2.810552374 (`tournament/gamma/phase4/nr_lyap.py`/`nr_ec2.py`/`nr_shoot_ec.py`). The Friedmann build retained as the `--friedmann` control face. **The contract's original gates fire unchanged — the module is COMPLETE.**
